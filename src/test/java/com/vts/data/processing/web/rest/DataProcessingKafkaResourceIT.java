...
class DataProcessingKafkaResourceIT {
    ...
    void setup() {
        ...
        DataProcessingKafkaResource kafkaResource = new DataProcessingKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/data-processing-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/data-processing-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
